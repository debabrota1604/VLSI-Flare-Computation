#include "routing.h"
extern int *Tree_pred;        //Tree_pred[j] is the predecessor node for j
extern TLAYER *Layer;           //layer array
extern TGLOBALNODE *GlobalNode; //global nodes array
extern TNET *Net_Tree;          //tree array
extern int Size_X, Size_Y;      //plane dimension
extern short Size_Z;            //number of layers
void
IMAD_q (int s, TNET * CurNet, float r_0, int N, int N_All, TNODE * Node_All,
        int MinResource, float q) 
//this procedure constructs one timing-driven congestion-aware Steiner tree
{
  int u, v, v1, v2, i, j, k, l, Sinks_in_Tree, dist;
  int Min_res, L, lev, number, Num_nodes;
  short lay, layer_u, layer_v;
  short Pred_dir;
  float Min_arc, cap, res, delay, C_0;
  short *In_Tree;             //If In_Tree[i]==1, then node i is in current tree
  short *In_Tree_tmp;           //If In_Tree[i]==1, then node i is temporary in current tree
  ...
  short *Up_Down;               //Up_Down[i]=up if i\in upper level; Up_Down[i]=down if i\in lover level
  ...
  C_last = (float *) calloc (N_All + 1, sizeof (float));
...
//------------------------------------------------------------------------------
  ...
  for (i = 1; i <= N_All; i++)
    if (Tree_pred[i] > 0)
      C_last[i] = Node_All[i].SubtreeCap;
  ...
  do                            //do1
  {
      ...
      do                        //do2
    {
      ...
      for (u = 1; u <= N_All; u++)
        ...
        {
          if (Node_All[u].w > 0 && In_Tree_tmp[Node_All[u].w] == 0)     //there is free (not in T)
          {
...
//------------------------------------------------------------------------------
          if (Node_All[u].n > 0 && In_Tree_tmp[Node_All[u].n] == 0)     //north
          {
            v = Node_All[u].n;
            Min_res = MaxInt;
            v1 = Node_All[u].No;
...
//------------------------------------------------------------------------------
          if (Node_All[u].e > 0 && In_Tree_tmp[Node_All[u].e] == 0)     //east
          {
            v = Node_All[u].e;
            Min_res = MaxInt;
            v1 = Node_All[u].No;
...
//------------------------------------------------------------------------------
          if (Node_All[u].s > 0 && In_Tree_tmp[Node_All[u].s] == 0)     //south
          {
            v = Node_All[u].s;
            Min_res = MaxInt;
            v1 = Node_All[u].No;
...
//------------------------------------------------------------------------------
          if (Node_All[u].u > 0 && In_Tree_tmp[Node_All[u].u] == 0)     //up
          {
            v = Node_All[u].u;
            Min_res = MaxInt;
            v1 = Node_All[u].No;
...
//------------------------------------------------------------------------------
          if (Node_All[u].d > 0 && In_Tree_tmp[Node_All[u].d] == 0)     //down
          {
            v = Node_All[u].d;
            Min_res = MaxInt;
            v1 = Node_All[u].No;
...
//recompute R[j]
        ...
        case west:
          R[j] =
            R[i] + Layer[Node_All[i].z].res * (Node_All[j].x - Node_All[i].x);
          cap = Layer[Node_All[i].z].cap * (Node_All[j].x - Node_All[i].x);
          break;
        case north:
          R[j] =
            R[i] + Layer[Node_All[i].z].res * (Node_All[i].y - Node_All[j].y);
          cap = Layer[Node_All[i].z].cap * (Node_All[i].y - Node_All[j].y);
          break;
        case east:
          R[j] =
            R[i] + Layer[Node_All[i].z].res * (Node_All[i].x - Node_All[j].x);
          cap = Layer[Node_All[i].z].cap * (Node_All[i].x - Node_All[j].x);
          break;
        case south:
          R[j] =
            R[i] + Layer[Node_All[i].z].res * (Node_All[j].y - Node_All[i].y);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          if (Node_All[j].type == 1)    //if sink is added
        {                       //if there are other sinks in the node j:
          for (k = 1; k <= N; k++)
          ...
          for (v = 1; v <= N_All; v++)
            if (In_Tree[v] != 1)
              ...
              C[v] = Node_All[v].cap;   //C[v] - total capacitance of subtree T_v                                     //of subtree T_v
...
//calculate C_j and d_{ij}. From leafs to root
            ...
            for (v = 1; v <= N_All; v++)
              if (level[v] == lev)
                ...
                case west:
                  res =
                    Layer[Node_All[u].z].res * (Node_All[v].x -
                                                Node_All[u].x);
                  cap =
                    Layer[Node_All[u].z].cap * (Node_All[v].x -
                                                Node_All[u].x);
                  break;
                case north:
                  res =
                    Layer[Node_All[u].z].res * (Node_All[u].y -
                                                Node_All[v].y);
                  cap =
                    Layer[Node_All[u].z].cap * (Node_All[u].y -
                                                Node_All[v].y);
                  break;
                case east:
                  res =
                    Layer[Node_All[u].z].res * (Node_All[u].x -
                                                Node_All[v].x);
                  cap =
                    Layer[Node_All[u].z].cap * (Node_All[u].x -
                                                Node_All[v].x);
                  break;
                case south:
                  res =
                    Layer[Node_All[u].z].res * (Node_All[v].y -
                                                Node_All[u].y);
                  cap =
                    Layer[Node_All[u].z].cap * (Node_All[v].y -
                                                Node_All[u].y);
                  break;
                case up:
                  res = 0;
  ...
  for (i = 1; i <= N_All; i++)
    if (Node_All[i].type == 1) //node i is sink
    {
      ...
      do {
        u = Tree_pred[v];
        Pred_dir = Node_All[v].pred_dir;
        while (degree[u] < 2 && Node_All[u].pred_dir == Pred_dir && Node_All[u].type < 0)     //u is intermediate and degree[u]=1
        {
          j = Tree_pred[u];
          Tree_pred[u] = -1;
          Tree_pred[v] = j;
...
/*
  for(v=1;v<=N_All;v++)
   ...
   else C[v]=Node_All[v].cap; //C[v] - total capacitance of subtree T_v
*/ 
//++++++++++++++++++++++++++++++end
//calculate C_j and d_{ij}. From leafs to root
    for (lev = L; lev > 0; lev--)
